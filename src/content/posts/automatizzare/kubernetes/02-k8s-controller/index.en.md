...
This schema — observe, compare, act — repeats in every corner of Kubernetes. The [Ingress Controller](/en/blog/automatizzare/kubernetes/01-article-ingress-k8s/) observes Ingress resources and reconfigures the reverse proxy. [Cluster API controllers](/en/blog/automatizzare/kubernetes/02-capi-part2-internals/) observe Custom Resources that describe clusters and machines, and reconcile the underlying infrastructure. The pattern is the same; only the observed resources and the actions taken differ.
...
* **[CAPI Part 1: From Chaos to Automation](/en/blog/automatizzare/kubernetes/01-capi-part1-intro/)**: The first article in the Cluster API series, which extensively uses the controller pattern.

* **[CAPI Part 2: Anatomy of Cluster API](/en/blog/automatizzare/kubernetes/02-capi-part2-internals/)**: Deep dive into the internal architecture of CAPI and its controllers.

* **[From port-forward to Ingress](/en/blog/automatizzare/kubernetes/01-article-ingress-k8s/)**: How the Ingress Controller works — another practical example of the pattern described in this article.
