...
In the [first article of this series](/posts/dotnet/linq/01-errori-produzione/) we examined four LINQ patterns found in a commercial fleet dispatcher. The fixes were straightforward — `ToHashSet`, `GroupBy`, `ToLookup`, `foreach`. But *how much* does it actually matter? Saying "we go from O(n²) to O(n)" is theoretically correct. In practice, the tech lead's question is different: "How many microseconds do we save? How much less memory? Is the refactoring worth it?" Today we put numbers to it.
