...
The rest of the article follows a precise itinerary. First the operational definitions of SLI, SLO and error budget are fixed, because without them the discussion that follows has nothing to stand on. Then we see why the static threshold is the wrong tool, how the SRE Workbook formalised multi-window multi-burn-rate alerting as the answer, and what the three canonical pairs to install in production are. In the later sections (part 6 onwards) we move to a Docker Compose demo showing the behaviour of the three pairs on a service with an injected error, and close with the typical traps and a practical selection table.
...
The third article of the series will tackle the complementary problem: **how to forecast the exhaustion of the error budget** using time-series techniques more sophisticated than `predict_linear`, to anticipate not only the saturation of a physical resource but the probable violation of the SLO in the days ahead. The goal is to close the circle between reactive alerting (threshold), proactive alerting on resources (`predict_linear`), alerting on the budget (burn-rate), and finally forecasting on the SLO.
