...
The [previous article](/posts/otel-website-material/05-management/) addressed the first production problem: **volume**. With tail sampling and retention, volume drops by 90% without losing visibility into errors. But a question remains: for the data that does survive, **where does it end up?**
...
Separating destinations is not enough: each destination needs a **persistence strategy** consistent with the type of data it receives. The [previous article](/posts/otel-website-material/05-management/) shows how to configure a single retention for all traces (Tempo, 7 days). With routing, you can apply different policies to each stream.
...
The persistence strategies integrate with the [tail sampling from the previous article](/posts/otel-website-material/05-management/) into a complete pipeline:
...
**Related articles:**
- [Tail Sampling and Retention](/posts/otel-website-material/05-management/) — Reducing volume before routing
